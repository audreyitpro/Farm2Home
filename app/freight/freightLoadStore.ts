...
function normalizeProof(proof: any): FreightLoadProof | undefined {
  if (!proof) return undefined;

  return {
    signerName: String(proof.signerName || ""),
    signature: String(proof.signature || ""),
    photoUri: String(proof.photoUri || ""),
    capturedAt: String(proof.capturedAt || new Date().toISOString()),
  };
}
...
function normalizeLoad(load: any): FreightLoad {
  ...
  const payoutAmount =
    load?.payoutAmount !== undefined
      ...
      : Number(miles * ratePerMile);

  const farmerName =
    load?.farmerName || load?.farmName || load?.postedBy || "Farm2Home Partner";

  const pickupLocation = load?.pickupLocation || load?.pickup || "";
  const dropoffLocation = load?.dropoffLocation || load?.dropoff || "";
  const equipment = load?.equipment || load?.requiredEquipment || "Not provided";
  const title =
    load?.title || load?.commodity || load?.loadType || "Farm2Home Freight Load";
  ...
  return {
    ...
    loadType: String(load?.loadType || "Other"),
    description: String(description),
    cargoDescription: String(description),
    ...
    pickupProof: normalizeProof(load?.pickupProof),
    deliveryProof: normalizeProof(load?.deliveryProof),

    createdAt: load?.createdAt || now,
    updatedAt: load?.updatedAt || now,
  };
}
...
export async function saveFreightLoads(loads: FreightLoad[]): Promise<void> {
  try {
    ...
    console.log("Save freight loads error:", error);
  }
}
...
export async function getFreightLoads(): Promise<FreightLoad[]> {
  ...
  const cleanSavedLoads = savedLoads.filter((load) => !starterIds.has(load.id));
...
export async function getCarrierLoads(carrierId: string): Promise<FreightLoad[]> {
  const loads = await getFreightLoads();
...
export async function deleteFreightLoad(loadId: string): Promise<FreightLoad[]> {
  const savedLoads = await getSavedFreightLoads();
